...
def deletethis():
    ...
    time.sleep(0.5)
    db.execute('delete from auto_login_sut where username={}'.format(username))
    db.commit()
def dbcount():
    global dn
    dn=0
    tableall = db.execute('select * from auto_login_sut')
...
def confirm():
    while 1==1:
        try:
            log = requests.get('http://172.16.254.2/auto_login_sut')
            html = html2text.html2text(log.text)
            check = checkuserpass()
            if check == 1:
                ...
                print('Username and Password is not match!\nTry to log out and auto_login_sut with new account!')
                time.sleep(2)
                try:
                    logout = requests.get('http://172.16.254.2/logout')
                    test()
...
def getup():
    global username,password
    table = db.execute('SELECT * FROM auto_login_sut ORDER BY RANDOM() LIMIT 1')
    for each in table:
...
def login():
    ...
    while (html == 'Cant Connect to Login Server!'):
        try:
            r = requests.post('http://172.16.254.2/auto_login_sut', data=data)
            import html2text
...
def test():
    ...
    if "If nothing happens," in html:
        ...
        confirm()
        f.close()
input("\n      This app developed by Jacob Zarg...")
os.system('cls')
test()
