...
table = db.execute('select ROWID, * from auto_login_sut')
for usr1 in table:
    table2 = db.execute('select ROWID, * from auto_login_sut')
    for usr2 in table2:
        if usr1['rowid'] != usr2['rowid']:
            #print(usr1['username'],usr2['username'])
            if usr1['username'] == usr2['username']:
                #print(usr1['rowid'],usr2['rowid'])
                db.execute('delete from auto_login_sut where rowid={}'.format(usr1['rowid']))
                rc+=1
db.commit()
